...
const validateOrderCreation = (req, res, next) => {
  ...
  // Validate each item (frontend may send item_name or name)
  for (let i = 0; i < items.length; i++) {
    ...
    // Sanitize item data (preserve name from either field for the route handler)
    const cleanedName = sanitizeString(itemName);
    items[i].item_name = cleanedName;
