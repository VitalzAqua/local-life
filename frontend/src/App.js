...
function App() {
  ...
  useEffect(() => {
    ...
    // Real-time driver tracking is now handled automatically by the DriverService
    console.log('🚀 App: New simplified delivery system initialized');
  ...
  const handleShowOrders = () => {
    console.log('📋 Orders button clicked, user:', user);
    if (user) {
      setSidebarView('orders');
      setSelectedStore(null);
      console.log('📋 Setting sidebar view to orders');
    } else {
      console.log('📋 User not logged in, cannot show orders');
    }
  };
  ...
  const handleLocationSet = async (locationData) => {
    ...
    console.log('Location set:', locationData.address);
  };
  ...
  const renderSidebarContent = () => {
    console.log('🎯 Rendering sidebar content for view:', sidebarView);
    switch (sidebarView) {
      case 'orders':
        console.log('🎯 Rendering OrderHistory component');
        return <OrderHistory user={user} />;
      case 'store':
        console.log('🎯 Rendering StoreDetails component for store:', selectedStore?.name);
      ...
      default:
        console.log('🎯 No sidebar content to render');
        return null;
    }
  };
