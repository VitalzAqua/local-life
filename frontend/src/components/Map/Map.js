        ...
        if (!foundMarker && matchingStore) {
          ...
          const popupContent = `
            <div class="store-popup">
              ...
              <button onclick="window.tempMarkerClick && window.tempMarkerClick()" class="btn btn-primary btn-sm">
                View Details
              </button>
            </div>
          `;
          
          // Set up temporary click handler
          window.tempMarkerClick = () => {
            onStoreSelect(matchingStore);
            map.removeLayer(tempMarker);
            delete window.tempMarkerClick;
          };
...
const Map = ({ userLocation, onStoreSelect, user }) => {
  ...
  // Debug categories
  useEffect(() => {
    console.log('🏷️ Categories in Map component:', categories);
  }, [categories]);
  ...
  const fetchDrivers = useCallback(async () => {
    ...
    try {
      ...
      if (isAdminMode) {
        ...
        fetchedDrivers = await apiService.getAllDriversAdmin(process.env.REACT_APP_ADMIN_CODE || '780523');
      } else if (user?.id) {
        console.log('🚗 Fetching user delivery drivers...');
  ...
  const handleAdminModeToggle = useCallback(() => {
    if (!isAdminMode) {
      // Prompt for password when activating admin mode
  ...
  const handleAdminPasswordSubmit = useCallback(async (password) => {
    try {
      ...
      // Test the password by attempting to fetch admin drivers
      await apiService.getAllDriversAdmin(password);
      
      // If successful, activate admin mode
      setIsAdminMode(true);
      setShowAdminPasswordModal(false);
  ...
  const handlePlaceSelect = useCallback((locationData) => {
    // When a place is selected from autocomplete, center map and search nearby stores
  ...
  // Legacy search handler (keeping for potential future use)
  // const handleSearch = useCallback((e) => {
  //   e?.preventDefault();
  //   searchStores(searchQuery, selectedCategories);
  // }, [searchQuery, selectedCategories, searchStores]);
  ...
  const handleLocationSelect = useCallback(async (location) => {
    try {
      ...
      if (!matchingStore) {
        const categoryStores = await apiService.getNearbyStores([location.category]);
  ...
  // Subscribe to driver updates - stable subscription with faster refresh
  useEffect(() => {
    if (!user?.id && !isAdminMode) return;

    console.log('🔧 Setting up driver subscription...');
    
    // Start real-time driver tracking
    const unsubscribe = driverService.startDriverSimulation();
    
    // More frequent driver position updates
    const refreshDrivers = async () => {
      try {
        let updatedDrivers = [];
        if (isAdminMode) {
          updatedDrivers = await apiService.getAllDriversAdmin(process.env.REACT_APP_ADMIN_CODE || '780523');
        } else if (user?.id) {
          // For regular users, get all drivers to see movement simulation
          updatedDrivers = await driverService.getAllDriversAdmin(process.env.REACT_APP_ADMIN_CODE || '780523');
        }
        
        // Only update if we have drivers and positions have changed
        if (updatedDrivers.length > 0) {
          setDrivers(updatedDrivers);
          console.log(`🔄 Updated ${updatedDrivers.length} driver positions`);
        }
      } catch (err) {
    ...
    // Set up rapid refresh interval for real-time movement
    const refreshInterval = setInterval(refreshDrivers, 3000); // Every 3 seconds
  ...
  // Update driver markers on the map - handled by DriverMarkers component
  // Removed direct map manipulation, now using React Leaflet pattern
