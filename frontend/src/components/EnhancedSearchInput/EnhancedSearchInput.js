...
const EnhancedSearchInput = ({ value, onChange, onSearch, onLocationSelect, placeholder = "Search for stores..." }) => {
  ...
  const searchStores = useCallback(async (searchQuery) => {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}/api/search/global?q=${encodeURIComponent(searchQuery)}&limit=20`,
  ...
  // Keep internal query in sync if the parent resets it (e.g. clear button)
  useEffect(() => {
