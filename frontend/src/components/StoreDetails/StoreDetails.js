...
const StoreDetails = ({ store, user }) => {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      // Start delivery simulation for delivery orders
      if (orderData.order_type === 'delivery') {
        try {
          const { default: deliveryService } = await import('../../services/deliveryService');
          await deliveryService.startDeliverySimulation(createdOrder.id);
        } catch (error) {
          console.error('Error starting delivery simulation:', error);
          // Don't fail the order if simulation fails
        }
      }
